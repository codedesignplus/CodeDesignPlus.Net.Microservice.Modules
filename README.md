...
5. Run the desired entry point:
   ...
   - For gRPC:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Modules.gRpc
      ```

   - For Worker:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Modules.AsyncWorker
      ```
...
1. Build the Docker image using the Dockerfile in the REST API entry point:
   ```bash
   docker build -t ms-modules-rest . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Modules.Rest/Dockerfile

   docker run -d -p 5000:5000 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-modules-rest ms-modules-rest
   ```

2. Build the Docker image using the Dockerfile in the gRPC entry point:
   ```bash
   docker build -t ms-modules-grpc . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Modules.gRpc/Dockerfile

   docker run -d -p 5001:5001 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-modules-grpc ms-modules-grpc
   ```

3. Build the Docker image using the Dockerfile in the Worker entry point:
   ```bash
   docker build -t ms-modules-worker . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Modules.AsyncWorker/Dockerfile

   docker run -d -p 5002:5002 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-modules-worker ms-modules-worker
   ```
