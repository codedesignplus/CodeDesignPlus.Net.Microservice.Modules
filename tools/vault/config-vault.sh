...
vault kv put -mount=security-codedesignplus-keyvalue ms-modules \
...
vault kv get -mount=security-codedesignplus-keyvalue ms-modules

# Write database configuration
echo "$newlines"
echo "${BLUE}7. Writing database configuration...${NC}"
vault write security-codedesignplus-database/config/db-ms-modules \
    plugin_name=mongodb-database-plugin \
    allowed_roles="ms-modules-mongo-role" \
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" \
    username="admin" \
    password="password"

vault write security-codedesignplus-database/roles/ms-modules-mongo-role \
    db_name=db-ms-modules \
    creation_statements='{ "db": "admin", "roles": [{ "role": "readWrite", "db": "db-ms-modules" }] }' \
    default_ttl="1h" \
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-modules-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-modules-rabbitmq-role \
    vhosts='{"/":{"write": ".*", "read": ".*", "configure": ".*"}}'

vault read security-codedesignplus-rabbitmq/creds/ms-modules-rabbitmq-role